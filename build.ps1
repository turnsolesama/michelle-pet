...
$appIcon = Join-Path $PSScriptRoot 'assets\icon\michele-dorm-refined.ico'
...
$arguments += "/resource:$(Join-Path $PSScriptRoot 'assets\companion\dorm-body-refined.png'),Companion.DormRefinedBody"
$arguments += "/resource:$(Join-Path $PSScriptRoot 'assets\companion\dorm-hands.png'),Companion.DormHands"
foreach ($identity in @('Classic','Dorm')) {
    foreach ($expression in @('Happy','Shy')) {
        $face = Join-Path $PSScriptRoot ("assets\companion\expressions\" + $identity.ToLowerInvariant() + '-' + $expression.ToLowerInvariant() + '.png')
        if (-not (Test-Path -LiteralPath $face)) { throw "Missing expression: $face" }
        $arguments += "/resource:$face,Expression.$identity.$expression"
    }
}
...
@{ version='0.3.8'; languages=@('zh-CN','en-US'); edition=$edition; bytes=(Get-Item $exe).Length; sha256=(Get-FileHash $exe -Algorithm SHA256).Hash } | ConvertTo-Json | Set-Content (Join-Path $outDir 'package.json') -Encoding UTF8
