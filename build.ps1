...
$outDir = Join-Path $PSScriptRoot 'releases\v0.2.2'
...
@{ version='0.2.2'; bytes=(Get-Item $exe).Length; sha256=(Get-FileHash $exe -Algorithm SHA256).Hash; published=$false } | ConvertTo-Json | Set-Content (Join-Path $outDir 'package.json') -Encoding UTF8
