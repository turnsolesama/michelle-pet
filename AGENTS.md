...
- Three companion styles across two outfits: run CompanionOutfitRegression with the EXE and an absolute private evidence directory. Preserve bare dorm hands versus uniform gloves, equal fixed palm scale per outfit, all key reach, style-switch placement and the prior normal skin on exit. Official dorm reference PNG stays local under assets/references and must never enter release packages.

- v0.3.8: run CompanionExpressionRegression against the release EXE with an absolute private evidence directory. Verify all three styles, face-only overlays, neutral restoration, translated interactions, blink toggle and stationary click/drag behavior. Generated expressions are feature-masked by CompanionExpressions.cs; never replace whole body frames for blinking.
